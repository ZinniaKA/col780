...
class SIFT(object):
    ...
    def visualize_keypoints(self):
        # Visualize keypoints on the input image
        plt.figure(figsize=(8, 8))
        plt.imshow(self.im, cmap='gray')
        plt.scatter(self.kp_pyr[0][:, 1], self.kp_pyr[0][:, 0], s=5, c='r', marker='o')
        plt.title('Keypoints')
        plt.axis('off')
        plt.show()

    def visualize_features(self):
         # Visualize local features (descriptors)
        plt.figure(figsize=(8, 8))
        for kp, feat in zip(self.kp_pyr[0], self.feats[0]):
            plt.plot(kp[1], kp[0], 'ro', markersize=5)
            plt.arrow(kp[1], kp[0], 5 * np.cos(feat[1]), 5 * np.sin(feat[1]), color='g', linewidth=1.5)
        plt.imshow(self.im, cmap='gray')
...
def generate_octave(init_level, s, sigma): 
  ...
  for _ in range(s+2): 
    ...
    # Resize the next level to have the same dimensions as the initial level
...
def generate_gaussian_pyramid(im, num_octave, s, sigma): 
  ...
  for _ in range(num_octave): 
    ...
    # octave =cv2.resize(octave,(_,540))
    pyr.append(octave) 
    im = octave[-3][::2, ::2] 
    # Resize the input image for the next octave
    # im_height, im_width = im.shape[:2]
    # im = cv2.resize(im, (im_width // 2, im_height // 2))
  return pyr
...
def assign_orientation(kps, octave, num_bins=36):
    ...
    for kp in kps:
        ...
        kernel_height, kernel_width = kernel.shape
        print(kernel_height, kernel_width,w)
        L = octave[...,s]
...
image1 = SIFT(cv2.imread("data/Images/Field/1.jpg"))
